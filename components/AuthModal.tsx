...
import { X, ShieldCheck, Check, AlertCircle, Loader2, ArrowLeft, Edit3, Save } from "lucide-react";
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const googleUserEmail = email.trim() || `user_${Date.now().toString().slice(-4)}@gmail.com`;
      const googleUserName = name.trim() || googleUserEmail.split("@")[0];
      ...
      const res = await fetch("/api/auth/signin", {
        ...
        body: JSON.stringify({
          ...
          password: "GoogleAuthAutoLogin2026_Secure!",
          isGoogle: true,
          name: googleUserName,
        }),
      });
      ...
      setErrorMsg("Google Sign-In failed. Please use standard email.");
    } finally {
          ...
          <div className="space-y-4">
            ...
            {mode !== "forgot" && (
              <button
                ...
                className="w-full py-2.5 bg-[#16161a] hover:bg-[#1f1f24] text-white border border-zinc-700/80 font-medium text-xs rounded-xl shadow-sm transition-all duration-150 active:scale-[0.98] flex items-center justify-center space-x-2.5"
              >
                {googleLoading ? (
            ...
            {mode !== "forgot" && (
              <div className="flex items-center my-3">
                ...
                <span className="px-3 text-[11px] text-zinc-500 font-medium">OR</span>
                <div className="flex-1 border-t border-zinc-800" />
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 mt-2 bg-white hover:bg-zinc-200 text-black font-semibold text-xs rounded-xl shadow-lg transition-all duration-150 active:scale-[0.98] flex items-center justify-center space-x-2"
              >
                {loading ? (
